...
/// Detect per-edge border insets for a bubble polygon.
///
/// Same scanning logic as `detect_inset` but returns independent insets for each side,
/// avoiding the uniform-inset problem where a thick top border forces excessive
/// left/right padding (or vice versa).
pub fn detect_edge_insets(img: &DynamicImage, polygon: &[[f64; 2]]) -> EdgeInsets {
    let (x1, y1, x2, y2) = polygon_bbox(polygon);
    let (img_w, img_h) = img.dimensions();
    ...
    let default = EdgeInsets::uniform(MIN_INSET);
    ...
    let step = 8u32;
    let mut top_w = Vec::new();
    let mut bot_w = Vec::new();
    let mut left_w = Vec::new();
    let mut right_w = Vec::new();
    ...
    for sy in (by1 + 4..by2.saturating_sub(4)).step_by(step as usize) {
        if let Some(w) = scan_edge(img, bx1, sy, 1, 0, scan_depth) {
            left_w.push(w as f64);
        }
    }
    for sy in (by1 + 4..by2.saturating_sub(4)).step_by(step as usize) {
        if let Some(w) = scan_edge(img, bx2, sy, -1, 0, scan_depth) {
            right_w.push(w as f64);
        }
    }

    EdgeInsets {
        top: edge_consensus(&mut top_w),
        bottom: edge_consensus(&mut bot_w),
        left: edge_consensus(&mut left_w),
        right: edge_consensus(&mut right_w),
    }
}

/// Compute consensus inset for one edge from its scan samples.
fn edge_consensus(widths: &mut Vec<f64>) -> f64 {
    if widths.len() < MIN_SAMPLES {
        return MIN_INSET;
    }
    widths.sort_by(|a, b| a.partial_cmp(b).unwrap());
    let idx = ((widths.len() as f64 * CONSENSUS_RATIO) as usize).min(widths.len() - 1);
    (widths[idx] + BORDER_PAD).clamp(MIN_INSET, MAX_BORDER + BORDER_PAD)
}
...
fn polygon_bbox(polygon: &[[f64; 2]]) -> (f64, f64, f64, f64) {
    crate::types::polygon_bbox(polygon)
}
