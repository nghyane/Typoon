...
from typoon.vision.masks import materialize, select_strategy
        ...
        async with runtime.page_gate:
            ...
            if min(h, w) < _MIN_PAGE_DIM:
                ...
                if runtime.recognizer is not None:
                    raw_groups = await runtime.recognizer.recognize(image, raw_groups, source_lang)

                # Seg-only CTD UNet for Japanese pages — runs parallel with
                # detect (detection already done), ~265ms on CoreML.
                ...
                filter_result = default_filter().evaluate(raw_groups, ctx)
                if filter_result.n_rejected:
                    logger.debug(
                        "scan page %d: filtered %d noise groups: %s",
                        index, filter_result.n_rejected,
                        [(g.text[:20], v.signals[0].scorer)
                         for g, v in filter_result.rejected],
                    )
                groups = filter_result.kept

                # 3. MASK — select strategy + materialize per group
                blocks = list(detection.blocks)
                groups_with_masks = _attach_masks(groups, blocks, image, bubble_mask)

                page_results[index] = _assemble_page(
                    index, w, h, groups_with_masks, detection.detected_lang,
                )
...
async def _seg_unet(
    ...
    """Run CTD UNet seg-only pass. Returns refined bubble_mask or None."""
    if not hasattr(runtime, "ctd_seg") or runtime.ctd_seg is None:
        return None
    try:
        return await asyncio.to_thread(runtime.ctd_seg.run, image)
    except Exception as e:
        logger.debug("ctd_seg failed: %s", e)
        return None
...
def _find_members(
    ...
    """Return blocks whose centre lies inside bbox."""
    x1, y1, x2, y2 = bbox
    return tuple(
        b for b in blocks
...
) -> list[tuple[BubbleGroup, tuple]]:
    """Select + materialize erase mask for each kept group.

    Returns list of (group, masks) — masks is tuple[TextMask, ...].
    Mask generation is the only place that needs the image; downstream
    (assemble, render) only needs the pixel data.
    """
    result = []
    for g in groups:
        members = _find_members(blocks, g.bbox)
        recipe  = select_strategy(g, image, bubble_mask=bubble_mask)
        masks   = materialize(recipe, g, members, image, bubble_mask=bubble_mask)
        result.append((g, masks))
    return result
...
def _assemble_page(
    ...
    width: int,
    height: int,
    groups_with_masks: list[tuple[BubbleGroup, tuple]],
    detected_lang: str | None,
) -> _PageOutput:
    """Pure data transform: (group, masks) pairs → domain types.

    No business logic here — noise filtering and mask generation happen
    upstream in scan_one. strip_noise_tokens is applied here as a last-
    pass inline cleanup (URL suffixes, logo emoji appended by OCR).
    """
    ...
    for i, (g, masks) in enumerate(groups_with_masks):
        ...
        bubble_masks.append(BubbleMasks(erase_masks=masks))
