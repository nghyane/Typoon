...
export async function loadModelBytes(
  ...
  emit: (status: CapabilityStatus) => void,
  name: string,
  ...
  if (cached) {
    try {
      ...
      console.warn('[browser-sdk] cached model failed verification; fetching network copy', error)
    }
  }

  throwIfAborted(signal)
  emit({ name, state: 'downloading', progress: { receivedBytes: 0, totalBytes: descriptor.sizeBytes, ratio: 0 } })
  const response = await fetch(descriptor.url, { mode: 'cors', signal })
  if (!response.ok) throw new Error(`model fetch failed: ${response.status}`)
  const bytes = await readResponseBytes(response, signal, progress => emit({ name, state: 'downloading', progress }))
  throwIfAborted(signal)
...
async function readCachedModel(cache: ModelAssetCache, key: string): Promise<ArrayBuffer | null> {
  try {
    ...
    console.warn('[browser-sdk] model cache read failed; fetching network copy', error)
    return null
  }
}
...
async function writeCachedModel(cache: ModelAssetCache, key: string, bytes: ArrayBuffer): Promise<void> {
  try {
    ...
    console.warn('[browser-sdk] model cache write failed; continuing with memory copy', error)
  }
}
