...
# New architecture
from .hybrid import HybridEraser  # noqa: E402
from .routing import (  # noqa: E402
    classify_masks, build_page_mask, inpaint_region, _is_uniform_background,
)
...
__all__ = [
    "AOTGANEraser", "MedianEraser",
    "HybridEraser",
    "InpaintBackend", "TeLeABackend", "AOTGANBackend",
    "RemoteInpaintBackend", "CfSd15InpaintBackend", "Flux2KleinInpaintBackend",
    "classify_masks", "build_page_mask", "inpaint_region", "_is_uniform_background",
]
...
class AOTGANEraser:
    ...
    def _erase_sync(self, canvas: np.ndarray, masks: list[TextMask]) -> None:
        ...
        for cluster in _cluster_masks(masks):
            # On uniform backgrounds (white balloon interior, spread < 30)
            # median fill is cleaner — AOT creates artifacts on flat colour.
            if _is_uniform_background(canvas, cluster):
                for m in cluster:
                    _erase_with_median(canvas, m)
                continue
            try:
...
def _is_uniform_background(
    canvas: np.ndarray,
    cluster: list[TextMask],
    spread_threshold: int = 30,
) -> bool:
    """True when the background inside the cluster bbox is uniform colour.

    Samples pixels inside each mask's bbox but OUTSIDE the mask pixels —
    i.e. the balloon interior background. This avoids picking up screentone
    or art from the broader page context.
    """
    ch, cw = canvas.shape[:2]
    bg_pixels = []
    for m in cluster:
        ...
        crop_mask   = m.image[sy:sy + (ty2 - ty1), sx:sx + (tx2 - tx1)]
        bg = crop_canvas[crop_mask == 0]
        bg_pixels.append(bg)

    if not bg_pixels:
        return False
    bg_all = np.concatenate(bg_pixels, axis=0)
    if len(bg_all) < 10:
        return False

    lum = (bg_all[:, 0].astype(np.int32) * 299
           + bg_all[:, 1].astype(np.int32) * 587
           + bg_all[:, 2].astype(np.int32) * 114) // 1000
...
def _sample_background(img: np.ndarray, mask: TextMask) -> tuple[np.ndarray, int]:
    ...
    # Clamp sampling region to image bounds first, then subslice mask
    # to match — without the clamp the boolean index sizes diverge when
    # the mask bleeds past the image edge.
    ry1 = max(mask.y, 0)
    rx1 = max(mask.x, 0)
    ry2 = min(mask.y + mh, ih)
    rx2 = min(mask.x + mw, iw)
    # offsets into the mask that correspond to the clamped region
    my_off = ry1 - mask.y
    mx_off = rx1 - mask.x
    region = img[ry1:ry2:2, rx1:rx2:2]
    mask_sub = mask.image[my_off:my_off + (ry2 - ry1):2, mx_off:mx_off + (rx2 - rx1):2]
    # Ensure shapes match (ceil-div rounding can still differ by 1)
    rh, rw = region.shape[:2]
    mask_sub = mask_sub[:rh, :rw]
