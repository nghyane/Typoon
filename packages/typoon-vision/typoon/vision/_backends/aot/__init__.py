    ...
    def _ensure_loaded(self):
        ...
        if self._use_coreml:
            try:
                ...
                logger.info("coremltools not available — falling back to ONNX backend")
                if not (self._dir / "aot-inpaint.onnx").exists():
                    raise FileNotFoundError(
                        f"Neither coremltools nor aot-inpaint.onnx available in {self._dir}"
                    )
