...
@tool
async def look_at(args: LookAtArgs, ctx, prepared, reader, keyed) -> ToolResponse:
    ...
    notes = await _look_at(ctx, prepared, reader, pages=args.pages, keys=args.keys, query=args.query, keyed=keyed)
    if not notes:
        return ToolResponse("No visual notes returned.")
    return ToolResponse("\n".join(f"#{k}: {v}" for k, v in notes.items()))


def make_look_at(ctx, prepared, reader, keyed) -> Tool:
    return look_at(ctx=ctx, prepared=prepared, reader=reader, keyed=keyed)
