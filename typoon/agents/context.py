...
from typoon.agents.brief import ChapterBrief, AddressRule, chapter_text
from typoon.agents.skills import LOAD_SKILL_TOOL, SkillLibrary
from typoon.agents.tools.brief import ChapterBriefArgs, make_submit_chapter_brief
from typoon.agents.tools.look_at import make_look_at
from typoon.agents.tools.search_knowledge import make_search_knowledge
from typoon.domain.prepared import Chapter as PreparedChapter
from typoon.domain.scan import BubbleKey
from typoon.llm.ir import ContentPart, Message, ToolCallMsg, ToolResponse
...
async def build_chapter_brief(
    ...
    """Analyze chapter text and produce a ChapterBrief. Raises on failure."""
    ...
    load_skill_tool = Tool(
        LOAD_SKILL_TOOL,
        _LoadSkillArgs,
        lambda args: _handle_load_skill(args, skills),
    )
...
async def _context_snapshot(ctx: TranslateCtx, sensitive: list[BubbleKey]) -> str:
    ...
    if glossary:
        parts.append(
            ...
            f"Call search_knowledge(scope=glossary, queries=[<term>]) to look up."
        )
    else:
        parts.append("## Glossary\nEmpty.")

    recent = await store.get_recent_chapter_briefs(ctx.project_id, before_chapter=ctx.chapter, limit=10)
    if recent:
        lines = [
    ...
    if sensitive:
        lines = [
            f'  p{bk.page_index} #{bk.key}: "{bk.source_text}"'
            for bk in sensitive[:20]
        ]
        parts.append(
            "## Address-sensitive bubbles\n"
            "These affect Vietnamese xưng hô/family pronouns. Storyboard images around the relevant pages are attached below; inspect local story context before deciding speaker/listener. "
            "If still uncertain from the storyboard, write bubble_notes as 'Uncertain speaker; use neutral phrasing' rather than guessing.\n"
            + "\n".join(lines)
        )
...
def _context_user_message(text: str, prepared: PreparedChapter, sensitive: list[BubbleKey], keyed: list[BubbleKey]) -> Message:
    if not sensitive:
    ...
    for target in sensitive[:2]:
        ...
        parts.append(ContentPart.of_text(
            f"--- Address-sensitive dialogue-neighborhood storyboard: inspect speaker/listener for #{target.key} ---"
        ))
        parts.append(ContentPart.of_image(encode_page_jpeg(storyboard)))
    return Message.user_parts(parts)


def _storyboard_image(prepared: PreparedChapter, ordered: list[BubbleKey], target: BubbleKey):
    try:
