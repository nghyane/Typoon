"""CLI entry point: typoon detect | typoon translate"""
...
async def _auth(site: str):
    connector = None
    for c in _get_connectors():
        if site in c.site_name or c.site_name in site:
            connector = c
            break
...
async def _interactive():
    """Browse ↔ pipeline loop in a single TUI shell."""
    ...
    try:
        while True:
            ...
            # Run pipeline in the same TUI
            tui.switch_to_pipeline()
            await _run_in_tui(  # sets project info on tui
                tui, input_str, result.force,
                result.from_ch, result.to_ch, paths,
            )

            # Wait for user to see results before going back
            await tui.wait_for_key()

            # Reload projects and loop back to browse
            projects, chapters_map = await load_projects()
            tui.set_projects(projects, chapters_map)
    finally:
        tui.stop()
...
async def _run_in_tui(tui, input_str: str, force: bool, from_ch: float, to_ch: float, paths):
    """Run the translation pipeline with the TUI as the hook."""
    from .adapters.sqlite_store import SqliteStore
    from .engine import Engine
    from .orchestrator import Orchestrator, ResumePolicy
    from .runner import ChapterRunner

    try:
        tui.log("[dim]Loading models…[/]")
        engine, config, paths = Engine.from_config(paths=paths)
        paths.ensure()
        ...
        if _is_url(input_str):
            project_id, chapters, download_q, download_count, _ = await _resolve_url(
                input_str, store, "", "", from_ch, to_ch, paths, tui,
            )
        else:
            project_id, chapters = await _resolve_path(
                Path(input_str), store, "", "", from_ch, to_ch, tui,
            )

        total_chapters = len(chapters) + download_count
        if not total_chapters:
            tui.log("[red]No chapters to translate.[/]")
            await store.close()
            return
        ...
        provider = f"{config.translation.provider}/{config.translation.model}"

        # Update TUI header with project info
        tui._project_name = name
        tui._project_lang = lang
        tui._project_provider = provider
        tui._project_total = total_chapters

        project_out = paths.output / name.lower().replace(" ", "-")

        def _on_chapter(ch: float, pages):
        ...
        result = await orchestrator.run(
            ...
            hook=tui, on_chapter=_on_chapter,
...
def translate(
    ...
    source_lang: str = typer.Option("", "--source-lang", "-s", help="Source language (first run only)"),
    target_lang: str = typer.Option("", "--target-lang", "-t", help="Target language (first run only)"),
    from_ch: float = typer.Option(0, "--from", help="Start from chapter number (inclusive)"),
    to_ch: float = typer.Option(0, "--to", help="End at chapter number (inclusive, 0=all)"),
    force: bool = typer.Option(False, "--force", help="Re-translate done chapters"),
):
...
async def _translate(
    input_str: str, source_lang: str, target_lang: str,
    from_ch: float, to_ch: float, force: bool,
):
    from .adapters.cli_hook import RichHook
    from .adapters.sqlite_store import SqliteStore
    from .engine import Engine
    ...
    try:
        ...
        if _is_url(input_str):
            project_id, chapters, download_q, download_count, _ = await _resolve_url(
                input_str, store, source_lang, target_lang, from_ch, to_ch, paths, hook,
            )
        else:
            project_id, chapters = await _resolve_path(
                Path(input_str), store, source_lang, target_lang, from_ch, to_ch, hook,
            )
        ...
        lang_display = f"{project['source_lang']} → {project['target_lang']}" if project else ""

        hook.log(
            f"[bold]{config.translation.provider}[/] / {config.translation.model}  "
            f"[cyan]{project_name}[/] {total_chapters} chapters "
            f"{lang_display}" + (" [yellow]force[/]" if force else "")
        )

        project_out = paths.output / project_name.lower().replace(" ", "-")
        ...
        result = await orchestrator.run(
            project_id,
            chapters=chapters or None,
            hook=hook, on_chapter=_on_chapter,
            policy=ResumePolicy(force=force),
            chapter_stream=download_q,
            total_hint=total_chapters,
        )
...
async def _resolve_url(url, store, source_lang, target_lang, from_ch, to_ch, paths, hook):
    """URL → connector discover → download ready chapters + start background download.

    Returns (project_id, chapters) where chapters already downloaded are returned
    immediately. Remaining chapters download in background via _download_worker.
    """
    from .adapters.local_source import LocalSource
    ...
    hook.log(
        f"[cyan]{info.suggested_title}[/] ({info.suggested_lang}) "
    ...
    # Return local chapters + a download stream for the rest.
    # Orchestrator processes local chapters while downloads continue in background.
    download_q: asyncio.Queue | None = None
    download_fail_count = [0]  # mutable ref for background task
    if to_download:
...
async def _download_worker(chapters, connector, series_dir, headers, store, project_id, queue, hook, fail_count):
    """Download chapters in background, push each to queue. None sentinel at end."""
    from .adapters.local_source import LocalSource
    from .downloader import download_images
...
async def _resolve_path(path, store, source_lang, target_lang, from_ch, to_ch, hook):
    """Local path → detect chapters → (project_id, chapters)."""
    from .adapters.local_source import LocalSource
...
def _save_pages(pages, out_dir: Path) -> int:
    ...
    # Save individual JPEGs (3-5x smaller than PNG, visually identical)
    ...
    try:
        ...
        pil_pages = [
            Image.fromarray(p.rendered) for p in rendered
        ]
