...
async def fetch_chapter_list(client: httpx.AsyncClient, slug: str) -> list[tuple[int, int]]:
...
class Progress:
    ...
    async def finish_chapter(self, ch_num: int, page_count: int, skipped: bool = False):
        async with self._lock:
            self.done += 1
            self.pages += page_count
            elapsed = time.time() - self.t0
            status = "skip" if skipped else f"{page_count}p"
            log(f"  [{self.done}/{self.total}] Ch.{ch_num:02d} {status}  ({elapsed:.0f}s)")


async def download_chapter(
    client: httpx.AsyncClient, slug: str,
    ch_num: int, ch_id: int, output_dir: Path,
    sem: asyncio.Semaphore, progress: Progress,
) -> int:
    async with sem:
        ch_dir = output_dir / f"ch{ch_num:03d}"
        ...
        async def with_timeout(ch, cid):
            try:
                return await asyncio.wait_for(
                    download_chapter(client, args.series, ch, cid, output, sem, progress),
                    timeout=60,
                )
            except asyncio.TimeoutError:
                await progress.finish_chapter(ch, 0)
                log(f"    ⚠ Ch.{ch:02d} timed out")
                return 0
