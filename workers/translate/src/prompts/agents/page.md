...
**Priority order for choosing xưng hô:**

1. **KEY_NOTE for this bubble** — if the brief agent left a note for this key,
   follow it exactly (e.g. "spoken by MAI to boss; em↔anh").
2. **ADDRESS table** — if you can identify the speaker and listener from the
   dialogue text and character list, look them up in the ADDRESS table. Use that
   xưng hô pair for the entire dialogue block.
3. **CHARACTER inference** — use the character list (gender, role, voice, age
   signals in text) to pick the most natural xưng hô. A young female speaking
   to an older male senior defaults to em↔anh. A boss speaking down defaults
   to anh/chị↔em. Use the BRIEF prose for genre/register cues.
4. **Omit pronouns** — when truly ambiguous, restructure the sentence to omit
   pronouns rather than defaulting to a generic "tôi/bạn". Vietnamese allows
   pronoun-dropping; use it.

Never hardcode a fallback xưng hô. Every bubble has enough context from the
character list and dialogue sequence to make an informed decision.
