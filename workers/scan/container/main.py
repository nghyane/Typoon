...
Key derivation must match `workers/shared/src/keys.ts` exactly:
  ...
  digest  = blake2s(payload, 5 bytes)
  key     = 7-char base32 of digest (custom alphabet)
...
def _make_key(job_id: int, page_index: int, idx: int, salt: int = 0) -> str:
    # Must mirror workers/shared/src/keys.ts:makeKey exactly. The TS side
    # passes job_id as a JS number (unquoted in JSON); we mirror by
    # passing a Python int. sort_keys=True yields {idx, job_id, page, salt}.
    payload = json.dumps(
...
def _assign_keys(job_id: int, page_index: int, count: int, used: set) -> list[str]:
    keys = []
    for idx in range(count):
        salt = 0
        while True:
            k = _make_key(job_id, page_index, idx, salt)
            if k not in used:
                break
            salt += 1
        used.add(k)
...
async def _scan_page(
    ...
    # Assign stable keys
    bubble_keys = _assign_keys(job_id, page_index, len(groups), used_keys)
...
async def _scan_impl(req: Request):
    raw_job_id = req.query_params.get("job_id")
    if not raw_job_id:
        raise HTTPException(400, "job_id required")
    try:
        job_id = int(raw_job_id)
    except ValueError:
        raise HTTPException(400, f"job_id must be int, got {raw_job_id!r}")
    ...
    log.info("scan job_id=%d pages=%d lang=%s", job_id, len(pages), lang_hint)
    t0      = time.perf_counter()
    ...
    results = await asyncio.gather(*[
        _scan_page(runtime, job_id, p["page_index"], p["prepared_key"],
                   lang_hint, used_keys)
        for p in pages
    ])
    ...
    log.info("scan done job_id=%d scan=%.1fs storyboard=%.1fs total=%.1fs",
             job_id, t_scan, t_sb, time.perf_counter() - t0)
