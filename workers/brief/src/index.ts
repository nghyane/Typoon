...
import type { BriefAddressPair, BriefCharacter, BriefIndex } from "@typoon/shared";
...
export class BriefService extends WorkerEntrypoint<Env> {
  ...
  private async persist(
    ...
    if (m.brief_prose) writes.push(
      putBytes(this.env.R2, r2keys.briefProse(String(args.chapter_id)),
        new TextEncoder().encode(m.brief_prose), "text/plain; charset=utf-8"));
    if (m.glossary.size > 0) writes.push(
      putJson(this.env.R2, r2keys.briefGloss(String(args.chapter_id)), Object.fromEntries(m.glossary)));
    if (m.characters.length > 0) writes.push(
      putJson(this.env.R2, r2keys.briefChars(String(args.chapter_id)), m.characters));
    if (m.address.length > 0) writes.push(
      putJson(this.env.R2, r2keys.briefAddress(String(args.chapter_id)), m.address));
    if (Object.keys(keyNotes).length > 0) writes.push(
      putJson(this.env.R2, r2keys.briefNotes(String(args.chapter_id)), keyNotes));
    if (m.noise.size > 0 || noisePages.length > 0) writes.push(
      putJson(this.env.R2, r2keys.briefNoise(String(args.chapter_id)),
    ...
    writes.push(putJson(this.env.R2, r2keys.brief(String(args.chapter_id)), index));
    await Promise.all(writes);
    index.timing_ms.total_ms = Date.now() - t0;

    return {
      index_key: r2keys.brief(String(args.chapter_id)), chunk_count: chunkCount,
