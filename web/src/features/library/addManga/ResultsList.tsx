// ResultsList — per-source grouped result rows.
//
// Pure presentation: emits `onPick(hit)` and lets the parent decide
// what "pick" means (add to library, attach as source, ...). The
// parent passes `pendingKey` to flag the row currently resolving and
// `pickedKeys` to flag rows the user has already acted on so they
// show a "đã chọn" check instead of being clickable again.
...
export function ResultsList({
  ...
  if (loading && hits.length === 0) {
    return (
      <div className="flex items-center gap-3 px-4 py-3 rounded-md bg-surface-2">
        <Spinner size={14} className="text-info-text" />
        ...
        {more > 0 && (
          <li>
            <button
              ...
              className={cn(
                'w-full inline-flex items-center justify-center gap-2 h-8 text-xs',
                'text-text-muted hover:bg-hover hover:text-text transition-colors cursor-pointer',
              )}
            >
              <ChevronDown size={12} />
              Xem thêm {more}
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          {manifest.languages.length > 0 && (
            <p className="text-xs text-text-subtle uppercase mt-1">
              {manifest.languages.slice(0, 3).join('/')}
            </p>
          )}
        </div>
        {pending && (
          <Spinner size={14} className="text-text-subtle shrink-0" />
        )}
        {picked && (
