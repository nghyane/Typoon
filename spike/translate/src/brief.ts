...
import { getBytes, getJson, keys as r2keys } from "../../shared/src/r2";
import type {
...
const EMPTY_BRIEF: LoadedBrief = {
  index: {
    chapter_id: "", version: 1, chunk_count: 0,
...
export async function loadBrief(
  r2: R2Bucket, chapter_id: string,
): Promise<LoadedBrief> {
  let index: BriefIndex;
  try {
    index = await getJson<BriefIndex>(r2, r2keys.brief(chapter_id));
  } catch {
  ...
  if (index.has_prose) {
    tasks.push(getBytes(r2, r2keys.briefProse(chapter_id))
      .then(b => { prose = new TextDecoder().decode(b); })
      .catch(() => { /* leave empty */ }));
  }
  if (index.has_gloss) {
    tasks.push(getJson<Record<string, string>>(r2, r2keys.briefGloss(chapter_id))
      .then(o => { glossary = new Map(Object.entries(o)); })
      .catch(() => {}));
  }
  if (index.has_chars) {
    tasks.push(getJson<BriefCharacter[]>(r2, r2keys.briefChars(chapter_id))
      .then(o => { characters = o; })
      .catch(() => {}));
  }
  if (index.has_address) {
    tasks.push(getJson<BriefAddressPair[]>(r2, r2keys.briefAddress(chapter_id))
      .then(o => { address = o; })
      .catch(() => {}));
  }
  if (index.has_notes) {
    tasks.push(getJson<Record<string, string>>(r2, r2keys.briefNotes(chapter_id))
      .then(o => { key_notes = o; })
      .catch(() => {}));
  }
  if (index.has_noise) {
    tasks.push(getJson<{ noise_keys: string[]; noise_pages: number[] }>(
      r2, r2keys.briefNoise(chapter_id),
    )
      .then(o => {
