...
import { assignKey } from "../../shared/src/keys";
...
import { parseTranslationReply, type TranslationOp } from "./parse";
import { callLLM, LLMFatalError, type LLMEnv } from "../../shared/src/llm";
import { loadBrief, briefSlice } from "./brief";

import { getJson, getBytes, putJson, keys as r2keys } from "../../shared/src/r2";
import { decodeMsgpack } from "../../shared/src/codec/msgpack";
import type { ScanPageResult, TranslateResult, TranslationOp, BlockClass } from "../../shared/src/types";
...
async function translateWindow(args: {
  ...
  sourceLang:  string;
  targetLang:  string;
}): Promise<{ ops: TranslationOp[]; meta: WindowMeta }> {
...
export class TranslateService extends WorkerEntrypoint<Env> {
  ...
  async translateChapter(args: {
    ...
    if (orderedKeys.length === 0) {
      ...
      const ok = r2keys.translate(args.chapter_id);
    ...
    const allOps = new Map<string, TranslationOp>(noiseOps);
